...
export default function InputField({
	...
	const errorClasses = error ? "border-red-500 shake" : "border-[var(--color-300)]";
	...
	return (
		...
		>
			{prefix && (
				<span className="bg-mono-700 text-mono-100 px-3 py-2 select-none">{prefix}</span>
			)}
			<input
				{...props}
				onChange={(e) => {
			...
			/>
			{suffix && <span className="flex items-center px-3 py-2 select-none">{suffix}</span>}
		</div>
	);
}
